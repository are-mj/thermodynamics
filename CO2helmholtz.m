function res_out = CO2helmholtz(T,v,max_order)
% Helmholtz molar free energy (a) and partial derivatives for pure CO2 
% Input:
%   T:   Temperature (K)
...
% Create phi_r as a sum of contributions phi_r = S1 + S2 + S3
  n = par.n(1:39);
  t = par.t;
  d = par.d;
  c = par.c;
  ...
  alpha = par.alpha;
  beta3 = par.beta(1:5);
  beta4 = par.beta(6);   % exploit that beta(6:8) = constant
  epsilon = par.epsilon;
  gamma = par.gamma;
  A = par.A(1);  % exploit that A(1:3) = constant
  B = par.B;
  C = par.C;
  D = par.D(1);  % exploit that D(1:3) = constant
  ...
  Delta = theta^2 + B.*y;
  Delta_t = -2*theta*ones(3,1);
  Delta_d = 2*theta*theta_d + B.*y_d;
  Delta_tt = 2*ones(3,1);
  Delta_td = -2*theta_d*ones(3,1);
